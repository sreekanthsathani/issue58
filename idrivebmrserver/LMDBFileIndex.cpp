...
void LMDBFileIndex::create(get_data_callback_t get_data_callback, void *userdata)
{
	...
	IDatabase *db=Server->getDatabase(Server->getThreadID(), IDRIVEBMRDB_SERVER_FILES_NEW);
...
bool LMDBFileIndex::create_env()
{
	...
	if(env==NULL)
	{
		...
		{
			std::auto_ptr<IFile> lmdb_f(Server->openFile("idrivebmr/fileindex/backup_server_files_index.lmdb", MODE_READ));
			if(lmdb_f.get()!=NULL)
			{
		...
		os_create_dir("idrivebmr/fileindex");
		...
		rc = mdb_env_open(env, "idrivebmr/fileindex/backup_server_files_index.lmdb", flags, 0664);
