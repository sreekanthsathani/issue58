...
void BackupServer::operator()(void)
{
	...
	ISettingsReader *settings=Server->createDBSettingsReader(Server->getDatabase(Server->getThreadID(),IDRIVEBMRDB_SERVER), "settings_db.settings",
	...
	{
		os_remove_nonempty_dir(tmpdir+os_file_sep()+"idrivebmr_tmp");
		if(!os_create_dir(tmpdir+os_file_sep()+"idrivebmr_tmp"))
		{
			Server->wait(5000);
			os_create_dir(tmpdir+os_file_sep()+"idrivebmr_tmp");
		}
		Server->setTemporaryDirectory(tmpdir+os_file_sep()+"idrivebmr_tmp");
	}
	else
	{
		...
		os_remove_nonempty_dir(w_tmp+os_file_sep()+"idrivebmr_tmp");
		if(!os_create_dir(w_tmp+os_file_sep()+"idrivebmr_tmp"))
		{
			Server->wait(5000);
			os_create_dir(w_tmp+os_file_sep()+"idrivebmr_tmp");
		}
		Server->setTemporaryDirectory(w_tmp+os_file_sep()+"idrivebmr_tmp");
	}
#endif
	...
	if( settings->getValue("backupfolder", &backupfolder) )
	{
		...
		{		
			std::string tmpfile_path=backupfolder+os_file_sep()+"idrivebmr_tmp_files";
...
#ifndef _WIN32
		os_create_dir("/etc/idrivebmr");
		writestring(backupfolder, "/etc/idrivebmr/backupfolder");
#endif
	}
	...
	while(true)
	{
		...
		{
			writestring("true", "idrivebmr/shutdown_now");
#ifdef _WIN32
...
void BackupServer::runServerRecovery(IDatabase * db)
{
	...
	std::string db_backupfolder = backupfolder + os_file_sep() + "idrivebmr";

	{
		...
		while (cur.next(res))
		{
			...
			if (!Server->fileExists("idrivebmr" + os_file_sep() + clientlist_fn))
			{
				...
				if (!bfile.empty())
				{
					...
					copy_file(bfile, "idrivebmr" + os_file_sep() + clientlist_fn);
				}
				else
				{
