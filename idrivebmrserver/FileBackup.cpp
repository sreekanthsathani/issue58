...
bool FileBackup::getTokenFile(FileClient &fc, bool hashed_transfer, bool request)
{
	...
	IFsFile *tokens_file=Server->openFile(os_file_prefix(backuppath_hashes+os_file_sep()+".idrivebmr_tokens.properties"), MODE_WRITE);
	if(tokens_file==NULL)
	{
		ServerLogger::Log(logid, "Error opening "+backuppath_hashes+os_file_sep()+".idrivebmr_tokens.properties", LL_ERROR);
		return false;
	}
	_u32 rc=fc.GetFile("idrivebmr/tokens_"+server_token+".properties", tokens_file, hashed_transfer, false, 0, false, 0);
	if(rc!=ERR_SUCCESS)
	{
	...
	std::auto_ptr<ISettingsReader> idrivebmr_tokens(
		Server->createFileSettingsReader(os_file_prefix(backuppath_hashes+os_file_sep()+".idrivebmr_tokens.properties")));

	std::string access_key;
	std::string client_access_key = server_settings->getSettings()->client_access_key;
	if(idrivebmr_tokens->getValue("access_key", &access_key) &&
		!access_key.empty() &&
...
{
	return "idrivebmr/clientlist_b_" + convert(ref_backupid) + ".ub";
}
	...
	while( (read=fileentries->Read(buffer, 4096, &has_read_error))>0 )
	{
		...
		for(size_t i=0;i<read;++i)
		{
			...
			if(b)
			{
				...
				if( !cf.isdir )
				{
					if (remote_path == "idrivebmr_backup_scripts"
						|| remote_path == "windows_components_config" 
						|| next(remote_path, 0, "idrivebmr_backup_scripts/") )
					{
						continue;
					}
...
void FileBackup::createUserViews(IFile* file_list_f)
{
	...
	if(idrivebmr_tokens.get()==NULL)
	{
		ServerLogger::Log(logid, "Cannot create user view. Token file not present.", LL_WARNING);
		return;
	}

	std::string s_real_uids = idrivebmr_tokens->getValue("real_uids", "");
	...
	for(size_t i=0;i<uids.size();++i)
	{
		...
		std::string s_gids = idrivebmr_tokens->getValue(uids[i]+".gids", "");
		...
		std::string accountname = base64_decode_dash(idrivebmr_tokens->getValue(uids[i]+".accountname", std::string()));
	...
	while((bread=file_list_f->Read(buffer, 4096))>0)
	{
		for(_u32 i=0;i<bread;++i)
		{
			...
			if(file_list_parser.nextEntry(buffer[i], data, &extra))
			{
				...
				if ( curr_path == os_file_sep() + "idrivebmr_backup_scripts"
					&& !data.isdir
...
void FileBackup::saveUsersOnClient()
{
	...
	std::string s_uids = idrivebmr_tokens->getValue("real_uids", "");
	std::vector<std::string> uids;
	...
	for(size_t i=0;i<uids.size();++i)
	{
		...
		backup_dao->addUserToken(accountname, clientid, idrivebmr_tokens->getValue(uids[i]+".token", std::string()));
		...
		for(size_t j=0;j<gids.size();++j)
		{
			std::string groupname = (base64_decode_dash(idrivebmr_tokens->getValue(gids[j] + ".accountname", std::string())));

			backup_dao->addUserTokenWithGroup(accountname, clientid, idrivebmr_tokens->getValue(gids[j]+".token", std::string()), groupname);
		}
	}

	std::vector<std::string> keys = idrivebmr_tokens->getKeys();
	for(size_t i=0;i<keys.size();++i)
	{
		if(keys[i].find(".token")==keys[i].size()-6)
		{
			backup_dao->addClientToken(clientid, idrivebmr_tokens->getValue(keys[i], std::string()));
		}
	}
}
