...
int check_files_index()
{
	...
	IDatabase *db=Server->getDatabase(Server->getThreadID(), IDRIVEBMRDB_SERVER_FILES);
	if(db==NULL)
	{
	...
	{
		ServerSettings server_settings(Server->getDatabase(Server->getThreadID(), IDRIVEBMRDB_SERVER));
