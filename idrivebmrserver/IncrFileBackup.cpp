...
bool IncrFileBackup::doFileBackup()
{
	...
	filesdao = new ServerFilesDao(Server->getDatabase(Server->getThreadID(), IDRIVEBMRDB_SERVER_FILES));
	...
	rc=fc.GetFile(group>0?("idrivebmr/filelist_"+convert(group)+".ub"):"idrivebmr/filelist.ub", tmp_filelist, hashed_transfer, false, 0, false, 0);
	if(rc!=ERR_SUCCESS)
	{
	...
	{
		clientlist_name="idrivebmr/clientlist_"+convert(clientid)+".ub";
	}
				...
				if(cf.isdir)
				{
					...
					if(cf.name!="..")
					{
						...
						if(!dir_linked && (!use_snapshots || indirchange || dir_diff) )
						{
							...
							if (depth == 0 && curr_path == "/idrivebmr_backup_scripts")
							{
							...
							if( !dir_diff && !indirchange && curr_path!="/idrivebmr_backup_scripts")
							{
			...
			for(size_t i=0;i<read;++i)
			{
				...
				if(b)
				{
					if(cf.isdir)
					{
						...
						if(cf.name!="..")
						{
							if(cf.name=="idrivebmr_backup_scripts")
							{
								script_dir=true;
							}
	...
	if (!ClientMain::run_script("idrivebmr" + os_file_sep() + "post_incr_filebackup",
				...
				{					
					...
					else
					{
						if( curr_dir_exists )
						{
							...
							if (curr_path == snapshot_path + os_file_sep() + "idrivebmr_backup_scripts")
							{
