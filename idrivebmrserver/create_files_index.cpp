...
bool create_files_index_common(FileIndex& fileindex, SStartupStatus& status)
{
	...
	IDatabase* db=Server->getDatabase(Server->getThreadID(), IDRIVEBMRDB_SERVER_FILES_DEL);
	...
	if(db->getEngineName()=="sqlite")
	{
		...
		if (FileExists("idrivebmr/backup_server_files.db-journal")
			|| FileExists("idrivebmr/backup_server_files.db-wal"))
		{
		...
		Server->deleteFile("idrivebmr/backup_server_files_new.db");

		Server->Log("Copying/reflinking database...", LL_INFO);
		if (!os_create_hardlink("idrivebmr/backup_server_files_new.db",
			"idrivebmr/backup_server_files.db", true, NULL))
		{
			Server->Log("Reflinking failed. Falling back to copying...", LL_DEBUG);

			if (!copy_file("idrivebmr/backup_server_files.db",
				"idrivebmr/backup_server_files_new.db"))
			{
		...
		str_map params;
		if (!Server->openDatabase("idrivebmr/backup_server_files_new.db", IDRIVEBMRDB_SERVER_FILES_NEW, params))
		{
			Server->Log("Couldn't open Database backup_server_files_new.db. Exiting. Expecting database at \"" +
				Server->getServerWorkingDir() + os_file_sep() + "idrivebmr" + os_file_sep() + "backup_server_files_new.db\"", LL_ERROR);
			return false;
		}

		Server->setDatabaseAllocationChunkSize(IDRIVEBMRDB_SERVER_FILES_NEW, sqlite_data_allocation_chunk_size);

		db_files_new = Server->getDatabase(Server->getThreadID(), IDRIVEBMRDB_SERVER_FILES_NEW);
		...
		db = Server->getDatabase(Server->getThreadID(), IDRIVEBMRDB_SERVER_FILES);
	...
	else
	{
		...
		std::auto_ptr<IFile> db_file(Server->openFile("idrivebmr/backup_server_files_new.db", MODE_RW));
		...
		if (!os_rename_file("idrivebmr/backup_server_files_new.db",
			"idrivebmr/backup_server_files.db"))
		{
...
{
	Server->deleteFile("idrivebmr/fileindex/backup_server_files_index.lmdb");
	Server->deleteFile("idrivebmr/fileindex/backup_server_files_index.lmdb-lock");
}
...
bool create_files_index(SStartupStatus& status)
{
	...
	if(!FileExists("idrivebmr/fileindex/backup_server_files_index.lmdb") || creating_index)
	{
		delete_file_index();
...
{
	if(!FileExists("idrivebmr/fileindex/backup_server_files_index.lmdb"))
	{
		return NULL;
	}
