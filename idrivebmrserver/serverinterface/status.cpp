...
bool client_download(Helper& helper, JSON::Array &client_downloads)
{
	...
	if(!FileExists("idrivebmr/IDriveBMRUpdate.exe"))
		return false;

	if(!FileExists("idrivebmr/IDriveBMRUpdate.sig2"))
		return false;
...
{
	std::auto_ptr<ISettingsReader> infoProperties(Server->createFileSettingsReader("idrivebmr/server_version_info.properties"));
	...
	else
	{
		...
		if (!server_settings->no_file_backups)
		{
				...
				&& !os_link_symbolic(os_file_prefix(testfolderpath), os_file_prefix(linkfolderpath)))
			{
				...
				ret.set("dir_error_hint", "IDriveBMR cannot create symbolic links on the backup storage. "
					"Your backup storage must support symbolic links in order for IDriveBMR to work correctly. "
					"The IDriveBMR Server must run as administrative user on Windows (If not you get error code 1314). "
		...
		if (!server_settings->no_file_backups)
		{
			...
			if (!use_tmpfiles)
			{
				tmpfile_path = server_settings->backupfolder + os_file_sep() + "idrivebmr_tmp_files";
			}
