			...
			for(size_t i=0;i<script_output.size();)
			{
				...
				else if (script_output[i] == 2)
				{
					if (i + 1 + sizeof(_u32) <= script_output.size())
					{
						...
						{
							if (!next(cfn, 0, "SCRIPT|idrivebmr/TAR"))
							{
								std::string tardirfn = ExtractFileName(getbetween("SCRIPT|", "|", cfn), "/");

								if (tardirfn != ".." && tardirfn != ".")
								{
									std::string tardirpath = backuppath + os_file_sep() + "idrivebmr_backup_scripts" + os_file_sep() + tardirfn;
									...
									tardirpath = backuppath_hashes + os_file_sep() + "idrivebmr_backup_scripts" + os_file_sep() + tardirfn;
							...
							std::string remote_fn = "idrivebmr/TAR|" + server_token + "|" + fn;
...
void ServerDownloadThread::base_dir_lost_hint()
{
	ServerLogger::Log(logid, "Hint: Current root backup path does not exist anymore. Usually this is because the current file "
		...
		"See https://www.idrivebmr.org/faq.html#base_dir_lost for details and for how to fix this issue", LL_INFO);
}
