...
namespace
{
	bool verify_signature(const std::string& exe_extension, const std::string& basename)
	{
...
#ifdef _WIN32
		std::string pubkey_fn="idrivebmr_ecdsa409k1.pub";
#else
		std::string pubkey_fn="idrivebmr/idrivebmr_ecdsa409k1.pub";
#endif
		std::string p_pubkey_fn = Server->getServerParameter("idrivebmr_public_key");
		...
		return crypto_fak->verifyFile(pubkey_fn, "idrivebmr/"+basename+"."+ exe_extension, "idrivebmr/"+ basename+".sig2");
	}
...
ACTION_IMPL(download_client)
{
		...
		|| (session!=NULL && (all_client_rights || !clientids.empty()) ) )
	{
			...
			std::find(clientids.begin(), clientids.end(), clientid)!=clientids.end() )
		{
			...
			std::string basename = "IDriveBMRUpdate";

			if (os == "osx" || os=="mac")
			{
				exe_extension = "sh";
				basename = "IDriveBMRUpdateMac";
			}
			else if (os == "linux")
			{
				exe_extension = "sh";
				basename = "IDriveBMRUpdateLinux";
			}
			...
			if(verify_signature(exe_extension, basename))
			{
				...
				std::string data=getFile("idrivebmr/"+basename+"."+ exe_extension);
				if( replaceStrings(helper, constructClientSettings(helper, clientid, clientname, authkey, access_keys), data) )
				{
					...
					Server->addHeader(tid, "Content-Disposition: attachment; filename=\"IDriveBMR Client ("+clientname+")."+exe_extension+"\"");
