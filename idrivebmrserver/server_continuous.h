...
#include "../idrivebmrcommon/change_ids.h"
#include <algorithm>
...
public:
	...
	void operator()()
	{
		server_settings.reset(new ServerSettings(Server->getDatabase(Server->getThreadID(), IDRIVEBMRDB_SERVER)));
		backupdao.reset(new ServerBackupDao(Server->getDatabase(Server->getThreadID(), IDRIVEBMRDB_SERVER)));
