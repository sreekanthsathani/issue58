...
#include "../idrivebmrcommon/InternetServiceIDs.h"
#include "../idrivebmrcommon/InternetServicePipe.h"
#include "../idrivebmrcommon/CompressedPipe2.h"
#include "../idrivebmrcommon/CompressedPipe.h"
...
#include <algorithm>
#include <assert.h>
#include "../idrivebmrcommon/InternetServicePipe2.h"
...
void InternetServiceConnector::Init(THREAD_ID pTID, IPipe *pPipe, const std::string& pEndpointName)
{
	...
	challenge=ServerSettings::generateRandomBinaryKey();
	{
		...
		unsigned int capa=0;
		ServerSettings server_settings(Server->getDatabase(pTID, IDRIVEBMRDB_SERVER));
...
void InternetServiceConnector::cleanup_pipes(bool remove_connection)
{
	...
	if(remove_connection
		&& client_data.find(clientname)!=client_data.end())
	{
...
{
	IDatabase *db=Server->getDatabase(tid, IDRIVEBMRDB_SERVER);
