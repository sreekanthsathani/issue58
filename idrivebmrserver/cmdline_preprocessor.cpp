...
void show_version()
{
	std::cout << "IDriveBMR Server v" << cmdline_version << std::endl;
...
int action_run(std::vector<std::string> args)
{
	...
	TCLAP::ValueArg<unsigned short> fastcgi_port_arg("f", "fastcgi-port",
		"Specifies the port where IDriveBMR server will listen for FastCGI connections",
		false, 55413, "port number", cmd);

	TCLAP::ValueArg<unsigned short> http_port_arg("p", "http-port",
		"Specifies the port where IDriveBMR server will listen for HTTP connections",
		false, 55414, "port number", cmd);

	TCLAP::ValueArg<std::string> logfile_arg("l", "logfile",
		"Specifies the log file name",
		false, "/var/log/idrivebmr.log", "path", cmd);
	...
	TCLAP::ValueArg<std::string> pidfile_arg("w", "pidfile",
		...
		false, "/var/run/idrivebmrsrv.pid", "path", cmd);
	...
	real_args.push_back(DATADIR "/idrivebmr/www");
	real_args.push_back("--idrivebmr_public_key");
	real_args.push_back(DATADIR "/idrivebmr/idrivebmr_ecdsa409k1.pub");
	real_args.push_back("--snapshot_helper");
	real_args.push_back(BINDIR "/idrivebmr_snapshot_helper");
	real_args.push_back("--mount_helper");
	real_args.push_back(BINDIR "/idrivebmr_mount_helper");
...
{
	TCLAP::CmdLine cmd("Rebuild IDriveBMR database", ' ', cmdline_version);
...
{
	TCLAP::CmdLine cmd("Try to repair IDriveBMR database", ' ', cmdline_version);
...
{
	TCLAP::CmdLine cmd("Decompress IDriveBMR compressed file", ' ', cmdline_version);
...
int action_mount_vhd(std::vector<std::string> args)
{
	...
	TCLAP::ValueArg<std::string> logfile_arg("l", "logfile",
		...
		false, "/var/log/idrivebmr-fuse.log", "path", cmd);
	...
	TCLAP::ValueArg<std::string> guestmount_user_arg("u", "guestmount-user",
		...
		false, "idrivebmr", "mount options", cmd);
...
void action_help(std::string cmd)
{
	...
	std::cout << "\t\t" "Run IDriveBMR server" << std::endl;
	std::cout << std::endl;
	...
	std::cout << "\t\t" "Try to repair IDriveBMR database" << std::endl;
	std::cout << std::endl;
	std::cout << "\t" << cmd << " defrag-database" << std::endl;
	std::cout << "\t\t" "Rebuild IDriveBMR database" << std::endl;
	std::cout << std::endl;
	...
	std::cout << "\t\t" "Decompress IDriveBMR compressed file" << std::endl;
	std::cout << std::endl;
