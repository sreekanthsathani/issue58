	...
	bool resume)
{
	...
	else if(!resume)
	{
		...
		if(script_cmd=="idrivebmr/FILE_METADATA")
		{
...
{
	if(public_fn.empty() || next(public_fn, 0, "idrivebmr/"))
	{
		return NULL;
	}
	...
	std::map<std::string, SPipeSession>::iterator it = pipe_files.find("idrivebmr/FILE_METADATA|"+server_token);
	...
	{
		Server->Log("No active metadata listener for \"idrivebmr/FILE_METADATA|" + server_token + "\"", LL_WARNING);
	}
...
void PipeSessions::transmitFileMetadata(const std::string & public_fn, const std::string& metadata, const std::string & server_token, const std::string & identity)
{
	...
	{
		Server->Log("No active metadata listener for \"idrivebmr/FILE_METADATA|" + server_token + "\" (raw)", LL_WARNING);
	}
}
...
void PipeSessions::transmitFileMetadataAndFiledataWait(const std::string & public_fn, const std::string & metadata, const std::string & server_token, const std::string & identity, IFile* file)
{
	...
	{
		Server->Log("No active metadata listener for \"idrivebmr/FILE_METADATA|" + server_token + "\" (send wait)", LL_WARNING);
	}
}
...
std::string PipeSessions::getKey( const std::string& cmd, int& backupnum, int64& fn_random)
{
	...
	if(script_cmd=="idrivebmr/FILE_METADATA" && cmd_toks.size()>1)
	{
