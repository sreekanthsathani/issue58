...
namespace backupaccess
{
	...
	STokens readTokens(const std::string& backupfolder, const std::string& clientname, const std::string& path)
	{
		...
		std::auto_ptr<ISettingsReader> backup_tokens(Server->createFileSettingsReader(backupfolder+os_file_sep()+clientname+os_file_sep()+path+os_file_sep()+".hashes"+os_file_sep()+".idrivebmr_tokens.properties"));
			...
			if( !fileaccesstokens || checkBackupTokens(*fileaccesstokens, backupfolder, clientname, backuppath) )
			{
				...
				if(backupid)
				{
					...
					for(size_t i=0;i<tfiles.size();++i)
					{
						...
						if(!tfiles[i].isdir)
						{
							if(path.empty() && (tfiles[i].name==".idrivebmr_tokens.properties" || next(tfiles[i].name, 0, ".symlink_")) )
								continue;
