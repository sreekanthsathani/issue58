...
#define LoadActions LoadActions_idrivebmrserver
#define UnloadActions UnloadActions_idrivebmrserver
#endif
...
#include "../idrivebmrcommon/chunk_hasher.h"
#include "LogReport.h"
...
#else
const std::string new_file="idrivebmr/new.txt";
#endif

void open_server_database(bool init_db)
{
	if( !FileExists("idrivebmr/backup_server.db") && init_db )
	{
		writestring(get_backup_server_db_data(), "idrivebmr/backup_server.db");
	}
	...
	if(! Server->openDatabase("idrivebmr/backup_server.db", IDRIVEBMRDB_SERVER, params) )
	{
		Server->Log("Couldn't open Database backup_server.db. Exiting. Expecting database at \"" +
			Server->getServerWorkingDir() + os_file_sep() + "idrivebmr" + os_file_sep() + "backup_server.db\"", LL_ERROR);
		exit(1);
	}
	...
	Server->setDatabaseAllocationChunkSize(IDRIVEBMRDB_SERVER, sqlite_data_allocation_chunk_size);

	if(!Server->getDatabase(Server->getThreadID(), IDRIVEBMRDB_SERVER))
	{
	...
	if (!Server->openDatabase("idrivebmr/backup_server_files.db", IDRIVEBMRDB_SERVER_FILES, params))
	{
		Server->Log("Couldn't open Database backup_server_files.db. Exiting. Expecting database at \"" +
			Server->getServerWorkingDir() + os_file_sep() + "idrivebmr" + os_file_sep() + "backup_server_files.db\"", LL_ERROR);
		exit(1);
	}
	...
	if (!Server->openDatabase("idrivebmr/backup_server_files.db", IDRIVEBMRDB_SERVER_FILES_DEL, params_nil))
	{
	...
	Server->setDatabaseAllocationChunkSize(IDRIVEBMRDB_SERVER_FILES, sqlite_data_allocation_chunk_size);

	if (!Server->getDatabase(Server->getThreadID(), IDRIVEBMRDB_SERVER_FILES))
	{
		Server->Log("Couldn't open backup server database. Exiting. Expecting database at \"" +
			Server->getServerWorkingDir() + os_file_sep() + "idrivebmr" + os_file_sep() + "backup_server_files.db\"", LL_ERROR);
		exit(1);
	...
	if (!Server->openDatabase("idrivebmr/backup_server_link_journal.db", IDRIVEBMRDB_SERVER_LINK_JOURNAL, params))
	{
		Server->Log("Couldn't open Database backup_server_link_journal.db. Exiting. Expecting database at \"" +
			Server->getServerWorkingDir() + os_file_sep() + "idrivebmr" + os_file_sep() + "backup_server_link_journal.db\"", LL_ERROR);
		exit(1);
	}
	...
	Server->setDatabaseAllocationChunkSize(IDRIVEBMRDB_SERVER_LINK_JOURNAL, sqlite_data_allocation_chunk_size);

	if (!Server->getDatabase(Server->getThreadID(), IDRIVEBMRDB_SERVER_LINK_JOURNAL))
	{
		Server->Log("Couldn't open backup server database. Exiting. Expecting database at \"" +
			Server->getServerWorkingDir() + os_file_sep() + "idrivebmr" + os_file_sep() + "backup_server_link_journal.db\"", LL_ERROR);
		exit(1);
	...
	if (!Server->openDatabase("idrivebmr/backup_server_links.db", IDRIVEBMRDB_SERVER_LINKS, params))
	{
		Server->Log("Couldn't open Database backup_server_links.db. Exiting. Expecting database at \"" +
			Server->getServerWorkingDir() + os_file_sep() + "idrivebmr" + os_file_sep() + "backup_server_links.db\"", LL_ERROR);
		exit(1);
	}
	...
	Server->setDatabaseAllocationChunkSize(IDRIVEBMRDB_SERVER_LINKS, sqlite_data_allocation_chunk_size);

	if (!Server->getDatabase(Server->getThreadID(), IDRIVEBMRDB_SERVER_LINKS))
	{
		Server->Log("Couldn't open backup server database. Exiting. Expecting database at \"" +
			Server->getServerWorkingDir() + os_file_sep() + "idrivebmr" + os_file_sep() + "backup_server_links.db\"", LL_ERROR);
		exit(1);
	...
	if (!Server->openDatabase("idrivebmr/backup_server_settings.db", IDRIVEBMRDB_SERVER_SETTINGS, params))
	{
	...
	Server->setDatabaseAllocationChunkSize(IDRIVEBMRDB_SERVER_SETTINGS, sqlite_data_allocation_chunk_size);

	if (!Server->getDatabase(Server->getThreadID(), IDRIVEBMRDB_SERVER_SETTINGS))
	{
		Server->Log("Couldn't open backup server database. Exiting. Expecting database at \"" +
			Server->getServerWorkingDir() + os_file_sep() + "idrivebmr" + os_file_sep() + "backup_server_settings.db\"", LL_ERROR);
		exit(1);
...
{
	std::string aname="idrivebmr/backup_server_settings.db";

	Server->attachToDatabase(aname, "settings_db", IDRIVEBMRDB_SERVER);	
}
...
void start_wal_checkpoint_threads()
{
	WalCheckpointThread* wal_checkpoint_thread = new WalCheckpointThread(100 * 1024 * 1024, 1000 * 1024 * 1024,
		"idrivebmr" + os_file_sep() + "backup_server_files.db", IDRIVEBMRDB_SERVER_FILES);
	...
	wal_checkpoint_thread = new WalCheckpointThread(10 * 1024 * 1024, 100 * 1024 * 1024,
		"idrivebmr" + os_file_sep() + "backup_server.db", IDRIVEBMRDB_SERVER, "main");
	Server->createThread(wal_checkpoint_thread, "main checkpoint");

	wal_checkpoint_thread = new WalCheckpointThread(10 * 1024 * 1024, 100 * 1024 * 1024,
		"idrivebmr" + os_file_sep() + "backup_server_settings.db", IDRIVEBMRDB_SERVER);
	Server->createThread(wal_checkpoint_thread, "settings checkpoint");

	wal_checkpoint_thread = new WalCheckpointThread(10 * 1024 * 1024, 100 * 1024 * 1024,
		"idrivebmr" + os_file_sep() + "backup_server_link_journal.db", IDRIVEBMRDB_SERVER_LINK_JOURNAL);
	...
	wal_checkpoint_thread = new WalCheckpointThread(100 * 1024 * 1024, 1000 * 1024 * 1024,
		"idrivebmr" + os_file_sep() + "backup_server_links.db", IDRIVEBMRDB_SERVER_LINKS);
	Server->createThread(wal_checkpoint_thread, "lnk checkpoint");
}

bool attach_other_dbs(IDatabase* db)
{
	if (!db->Write("ATTACH DATABASE 'idrivebmr/backup_server_files.db' AS files_db"))
	{
		return false;
	}

	if (!db->Write("ATTACH DATABASE 'idrivebmr/backup_server_link_journal.db' AS link_journal_db"))
	{
		return false;
	}

	if (!db->Write("ATTACH DATABASE 'idrivebmr/backup_server_links.db' AS links_db"))
	{
		return false;
	}
...
DLLEXPORT void LoadActions(IServer* pServer)
{
	...
	if((server_identity=getFile("idrivebmr/server_ident.key")).size()<5)
	{
		Server->Log("Generating Server identity...", LL_INFO);
		std::string ident="#I"+ServerSettings::generateRandomAuthKey(20)+"#";
		writestring(ident, "idrivebmr/server_ident.key");
		server_identity=ident;
	}

	if(FileExists("idrivebmr/server_ident_ecdsa409k1.pub") && crypto_fak!=NULL)
	{
		std::string signature;
		if(!crypto_fak->signData(getFile("idrivebmr/server_ident_ecdsa409k1.priv"), "test", signature))
		{
			Server->Log("Server ECDSA identity broken. Regenerating...", LL_ERROR);
			Server->deleteFile("idrivebmr/server_ident_ecdsa409k1.pub");
			Server->deleteFile("idrivebmr/server_ident_ecdsa409k1.priv");
		}
	}
	...
	if(!FileExists("idrivebmr/server_ident_ecdsa409k1.pub") && crypto_fak!=NULL)
	{
		...
		crypto_fak->generatePrivatePublicKeyPair("idrivebmr/server_ident_ecdsa409k1");
	}

	if(FileExists("idrivebmr/server_ident.pub") && crypto_fak!=NULL)
	{
		std::string signature;
		if(!crypto_fak->signDataDSA(getFile("idrivebmr/server_ident.priv"), "test", signature))
		{
			Server->Log("Server DSA identity broken. Regenerating...", LL_ERROR);
			Server->deleteFile("idrivebmr/server_ident.pub");
			Server->deleteFile("idrivebmr/server_ident.priv");
		}
	}
	...
	if(!FileExists("idrivebmr/server_ident.pub") && crypto_fak!=NULL)
	{
		...
		crypto_fak->generatePrivatePublicKeyPairDSA("idrivebmr/server_ident");
	}
	if((server_token=getFile("idrivebmr/server_token.key")).size()<5)
	{
		Server->Log("Generating Server token...", LL_INFO);
		std::string token=ServerSettings::generateRandomAuthKey(20);
		writestring(token, "idrivebmr/server_token.key");
		server_token=token;
	}

	Server->deleteFile("idrivebmr/shutdown_now");
	...
	if( FileExists("idrivebmr/backupfolder")
#ifndef _WIN32
		|| FileExists("/etc/idrivebmr/backupfolder")
#endif
		)
	{
		...
		if(res.empty())
		{
			...
			std::string bf=getFile("idrivebmr/backupfolder");
#ifndef _WIN32
			if (bf.empty())
			{
				bf = getFile("/etc/idrivebmr/backupfolder");
			}
#endif
			if(linecount(bf)>0)
				bf=getline(0, bf);
	...
	Server->Log("Started IDriveBMR...", LL_INFO);
	...
	{
		ServerSettings settings(Server->getDatabase(Server->getThreadID(), IDRIVEBMRDB_SERVER));
	...
	Server->Log("IDriveBMR Server start up complete.", LL_INFO);
}
...
DLLEXPORT void UnloadActions(void)
{
	...
	db_ids.push_back(IDRIVEBMRDB_SERVER);
	db_ids.push_back(IDRIVEBMRDB_SERVER_FILES);
	db_ids.push_back(IDRIVEBMRDB_SERVER_LINKS);
	db_ids.push_back(IDRIVEBMRDB_SERVER_LINK_JOURNAL);
...
bool upgrade35_36()
{
	...
	std::auto_ptr<IFile> db_file(Server->openFile("idrivebmr/backup_server.db", MODE_READ));

	if(db_file.get() && Server->getServerParameter("override_freespace_check")==std::string())
	{
		int64 free_space = os_free_space(Server->getServerWorkingDir()+os_file_sep()+"idrivebmr");

		int64 missing_free_space = db_file->Size()-free_space;
		if(free_space<db_file->Size())
		{
			Server->Log("IDriveBMR needs "+PrettyPrintBytes(missing_free_space)+" more free space for the upgrade process. Please free up this amount of space at \""+Server->getServerWorkingDir()+os_file_sep()+"idrivebmr\"", LL_ERROR);
			return false;
		}
		...
		if(!tmpdir.empty())
		{
			...
			if(free_space<db_file->Size())
			{
				Server->Log("IDriveBMR needs "+PrettyPrintBytes(missing_free_space)+" more free space for the upgrade process at the temporary location. "
...
void upgrade(void)
{
	...
	if(qp==NULL)
	{
		...
		db->Import("idrivebmr/backup_server.dat");
	...
	do
	{
		...
		if(ver>max_v)
		{
			Server->Log("Current IDriveBMR database version is "+convert(ver)+". This IDriveBMR"
				" server version only supports databases up to version "+convert(max_v)+"."
				" You need a newer IDriveBMR server version to work with this database.", LL_ERROR);
			exit(4);
		}
		db->BeginWriteTransaction();
		old_v=ver;
