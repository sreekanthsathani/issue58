...
std::string PipeFileTar::buildCurrMetadata()
{
	...
	data.addString(type + "idrivebmr_backup_scripts/"+ output_fn + (fn.empty() ? "" : "/"+ fn) );
	...
	while (true)
	{
		if(switchNext(fn, is_dir, is_symlink, is_special, symlink_target, size))
		{
			...
			while ( (slash_pos=fn.find('/', slash_pos+1))!=std::string::npos)
			{
				...
				{
					std::string public_fn = "idrivebmr_backup_scripts/" + output_fn + (csubdir.empty() ? "" : ("/" + csubdir));
			...
			std::string public_fn = "idrivebmr_backup_scripts/" + output_fn + (fn.empty() ? "" : ("/" + fn));
				...
				|| (starttime - Server->getTimeMS())>20000 )
			{
				...
				std::string remote_fn = "idrivebmr_backup_scripts/" + output_fn + (fn.empty() ? "" : ("/" + fn)) + "|" + convert(backupnum) + "|" + convert(fn_random) + "|" + server_token;
