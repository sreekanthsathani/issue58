...
function change_lang_select()
{
	...
	document.cookie="idrivebmr_lang="+cookie_lang+"; expires="+d.toUTCString()+"";
	if(window.localStorage)
	{
		window.localStorage.setItem('idrivebmr_lang', cookie_lang);
	}
...
function try_anonymous_login(data)
{
	...
	if(g.startup)
	{
		...
		cookie_lang = getCookie("idrivebmr_lang");
		
		if(!cookie_lang && window.localStorage)
		{
			cookie_lang = localStorage.getItem("idrivebmr_lang");
		}
...
function show_status2(data)
{
		...
		&& (typeof data.dir_error_show === "undefined" || data.dir_error_show===true)  )
	{
		...
		if(data.dir_error_hint)
		{
			...
			{
				ext_text+="<br>The entire drive is not accessible. If this is a network drive be aware that network drives are per user and IDriveBMR server runs as \"Local System\" user per default so it wont see your network drive. In this case you should use the UNC notation instead (\\\\servername\\share).";
			}
			else if(data.dir_error_hint=="folder_unc_access_denied")
			{
				ext_text+="<br>IDriveBMR is denied access to the network share, probably because it does not have the correct credentials to access the server. To setup IDriveBMR server to backup to a network share please see the FAQ: <a href=\"https://www.idrivebmr.org/faq.html#use_shares\">https://www.idrivebmr.org/faq.html#use_shares</a>";
			}
			else if(data.dir_error_hint=="dos_names_created")
			{
				ext_text+="<br>MS-DOS 8.3 compatibility names are created on the backup storage. This can lead to problems. You can disable 8.3 name generation by runnning <br>"
					+ "<code><pre>fsutil 8dot3name set "+data.dir_error_volume+" 1</pre></code><br>as administrator. IDriveBMR will stop showing this error after you run this command.";
			}
			else
			{
...
function show_live_log()
{
	...
	var live_log_page = dustRender("live_log", {session: g.session, clientid: clientid, 
												...
												idrivebmr_functions_js: I("idrivebmr_functions_js").src});
...
function aboutIDriveBMR()
{
	if(!startLoading()) return;
	stopLoading();
	
	var ndata=dustRender("about_idrivebmr", {version: I('server_version_full').innerHTML.trim()});
	if(g.data_f!=ndata)
	{
		$("#data_f").empty();
		I('data_f').innerHTML=ndata;
