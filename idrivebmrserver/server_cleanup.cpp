...
void ServerCleanupThread::operator()(void)
{
	...
	if(cleanup_action.action!=ECleanupAction_None)
	{
		...
		filesdao.reset(new ServerFilesDao(Server->getDatabase(Server->getThreadID(), IDRIVEBMRDB_SERVER_FILES)));
	...
	if (FileExists("idrivebmr/migrate_storage_to"))
	{
		std::string migrate_storage_to = trim(getFile("idrivebmr/migrate_storage_to"));
		if (!migrate_storage_to.empty())
		{
			while (true)
			{
				...
				bool ignore_copy_errors = FileExists("idrivebmr/migrate_storage_to.ignore_copy_errors");

				if (copy_storage(migrate_storage_to, ignore_copy_errors) == 0)
				{
					writestring("done", "idrivebmr/migrate_storage_to.done");
				}
...
void ServerCleanupThread::do_remove_unknown(void)
{
	...
	ServerLinkDao link_dao(Server->getDatabase(Server->getThreadID(), IDRIVEBMRDB_SERVER_LINKS));
...
bool ServerCleanupThread::backup_database(void)
{
	...
	if(settings.getSettings()->backup_database)
	{
		...
		copy_backup_ids.push_back(IDRIVEBMRDB_SERVER);
		copy_backup_ids.push_back(IDRIVEBMRDB_SERVER_SETTINGS);
		copy_backup_ids.push_back(IDRIVEBMRDB_SERVER_FILES);
		copy_backup_ids.push_back(IDRIVEBMRDB_SERVER_LINKS);
		copy_backup_ids.push_back(IDRIVEBMRDB_SERVER_LINK_JOURNAL);
		...
		if(integrity_ok)
		{
			std::string bfolder=settings.getSettings()->backupfolder+os_file_sep()+"idrivebmr";
			...
			for (size_t i = 0; i < copy_backup_ids.size(); ++i)
			{
				...
				WalCheckpointThread::lockForBackup("idrivebmr" + os_file_sep() + copy_backup[i]);
				...
				std::string errmsg1;
				bool copy_ok = copy_db_file(Server->getServerWorkingDir()+ os_file_sep() + "idrivebmr" + os_file_sep() + copy_backup[i],
				...
				if (copy_ok)
				{
					...
					std::string errmsg2;
					copy_ok = copy_db_file(Server->getServerWorkingDir() + os_file_sep() + "idrivebmr" + os_file_sep() + copy_backup[i]+"-wal",
						bfolder + os_file_sep() + copy_backup[i]+"-wal~", &backup_progress_wal, errmsg2, true);

					if (!copy_ok)
					{
						ServerLogger::Log(logid, "Backing up database failed. Copying idrivebmr" + os_file_sep() + copy_backup[i] + "-wal to " + bfolder + os_file_sep() + copy_backup[i] + "-wal~ failed. "+ errmsg2, LL_ERROR);
						total_copy_ok = false;
					}
					else
					{
				...
				{
					ServerLogger::Log(logid, "Backing up database failed. Copying idrivebmr" + os_file_sep() + copy_backup[i] + " to " + bfolder + os_file_sep() + copy_backup[i] + "~ failed. "+errmsg1, LL_ERROR);
					total_copy_ok = false;
				}

				WalCheckpointThread::unlockForBackup("idrivebmr" + os_file_sep() + copy_backup[i]);
			}	
		...
		else
		{
			...
			ClientMain::sendMailToAdmins("Database integrity check failed", "Database integrity check failed before database backup. You should restore the IDriveBMR database from a backup or try to repair it.");
			return false;
		}
	}
	...
	bool deep_check)
{
	...
	const std::string pool_root = backupfolder + os_file_sep() + clientname + os_file_sep() + ".directory_pool";
	IDatabase* db_links = Server->getDatabase(Server->getThreadID(), IDRIVEBMRDB_SERVER_LINKS);
...
bool ServerCleanupThread::backup_clientlists()
{
	...
	if(settings.getSettings()->backup_database)
	{
		...
		std::string srcfolder = Server->getServerWorkingDir()+os_file_sep()+"idrivebmr";
